...
export function prettyTextToString(txt: text.AnnotatedText) : string {
  ...
  // `soliditytop` loves outputting non-breaking spaces instead of normal whitespace, so we replace them
  ...
  if(psm) {
    psm.activate()
      .then(() => {
        ...
        subscriptions.push(psm.exports.registerSubstitutions({language: 'solidity', substitutions: [{ugly: 'SOLIDITY', pretty: '⫝'}]}));
      });
  }
