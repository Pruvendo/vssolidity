...
import {SoliditySettings} from './protocol';
...
export const regExpQualifiedSolidityIdent = /((\p{L}|[_\u00A0])(\p{L}|[0-9_\u00A0'])*)(\.((\p{L}|[_\u00A0])(\p{L}|[0-9_\u00A0'])*))*/u;
...
export function reload() : vscode.Disposable {
  ...
  const settings = vscode.workspace.getConfiguration('solidity') as any as SoliditySettings;
  ...
  subscriptions.push(vscode.languages.setLanguageConfiguration('solidity', {
    indentationRules: { increaseIndentPattern: increaseIndentRE, decreaseIndentPattern: matchNothing },
    wordPattern: regExpQualifiedSolidityIdent,
  }))
  ...
  if(settings.format.enable) {
    ...
    if (editProviders.length > 0) {
      subscriptions.push(vscode.languages.registerOnTypeFormattingEditProvider('solidity', {
